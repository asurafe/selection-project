...
const initHasSpuData = async (spu: SpuData) => {
  ...
  const res: [
    ...
    reqAllTradeMark(),
    reqSpuImageList(spu.id as number),
    reqSpuHasSaleAttr(spu.id as number),
    reqAllSalAttr(),
  ]);
  console.log(res);
  AllTradeMark.value = res[0].data;
  imgList.value = res[1].data.map((item) => {
    return {
      name: item.imgName,
      url: item.imgUrl,
    };
...
const initAddSpu = async (c3Id: number | string) => {
  ...
  const res: [AllTradeMark, HasSaleAttrResponseData] = await Promise.all([
    reqAllTradeMark(),
    reqAllSalAttr(),
  ]);
  AllTradeMark.value = res[0].data;
